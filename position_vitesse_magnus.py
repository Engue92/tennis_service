...
def calcule_position_vitesse_magnus(PXD, PYD, PZD, VXD, VYD, VZD, teste=False, VRD=0.4):
    ...
    for i in range(n-1):  # boucle le nombre d'iteration 
        ...
        T[i+1] = T[i]+h   # incrementation du temps en fonction du pas

        if sol != True :  # calcule de la trajectoir avant de toucher le sol
        
            # Calcule des approximation avec Euler
            (PX1,PY1,PZ1,VX1,VY1,VZ1) = eu.euler_methode(T[i],PX[i],PY[i],PZ[i],VX[i],VY[i],VZ[i],h,fct="m",VR=VRD)
            ...
            while abs(PX[i+1] - TAPX) > 0.000001 and abs(PY[i+1] - TAPY) > 0.000001 and abs(PZ[i+1] - TAPZ) > 0.000001 and abs(VX[i+1] - TAVX) > 0.000001 and abs(VY[i+1] - TAVY) > 0.000001 and abs(VZ[i+1] - TAVZ) > 0.000001 :
                ...
                (PX[i+1],PY[i+1],PZ[i+1],VX[i+1],VY[i+1],VZ[i+1]) = am.adams_moulton_methode(T[i],PX[i],PY[i],PZ[i],VX[i],VY[i],VZ[i],PX1,PY1,PZ1,VX1,VY1,VZ1,h,fct="m",VR=VRD)
